...
class ChangePasswordViewController: FormPopoverViewController {
    ...
    func textFieldShouldReturn(_ textField: UITextField) -> Bool {
        ...
        case oldPasswordTextField:
            let _ = self.newPasswordTextField.becomeFirstResponder()
        case newPasswordTextField:
            let _ = self.newPasswordTextField.resignFirstResponder()
        default:
    ...
    @IBAction func saveButtonTapped(_ sender: Any) {
        if self.oldPasswordTextField.isFirstResponder { self.oldPasswordTextField.resignFirstResponder() }
        if self.newPasswordTextField.isFirstResponder { self.newPasswordTextField.resignFirstResponder() }

        if !isValidOldPassword { self.oldPasswordTextField.setError(message: oldPasswordErrorMessage) }
        if !isValidNewPassword { self.oldPasswordTextField.setError(message: newPasswordErrorMessage) }

        if !isValidOldPassword || !isValidNewPassword { return }

        let sessionManager = SessionManager()

//        sessionManager.changePassword() {[weak self] in
